...
import com.vmg265.fastdrawplus.fork.categoryorder.CategoryOrderAdapter;
import com.vmg265.fastdrawplus.fork.categoryorder.ReorderHelperCallback;
...
public class SettingsActivity extends AppCompatPreferenceActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public static class AppearancePreferenceFragment extends PreferenceFragment {
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            android.preference.ListPreference iconPackPref = (android.preference.ListPreference) findPreference("globalIconPack");
            if (iconPackPref != null) {
                com.vmg265.fastdrawplus.fork.iconpack.IconPackManager iconPackManager = com.vmg265.fastdrawplus.fork.iconpack.IconPackManager.getInstance(getActivity());
                java.util.List<String> packs = iconPackManager.getAvailableIconPacks();
                CharSequence[] entries = new CharSequence[packs.size() + 1];
                CharSequence[] entryValues = new CharSequence[packs.size() + 1];
                entries[0] = getString(R.string.system_default);
                entryValues[0] = "";
                for (int i = 0; i < packs.size(); i++) {
                    entries[i + 1] = iconPackManager.getIconPackLabel(packs.get(i));
                    entryValues[i + 1] = packs.get(i);
                }
                iconPackPref.setEntries(entries);
                iconPackPref.setEntryValues(entryValues);
                bindPreferenceSummaryToValue(iconPackPref);
            }

            android.preference.Preference iconSizePref = findPreference("iconSizeMultiplier");
            if (iconSizePref != null) {
                bindPreferenceSummaryToValue(iconSizePref);
            }
        }
    }
